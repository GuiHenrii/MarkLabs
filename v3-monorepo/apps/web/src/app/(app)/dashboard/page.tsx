...
const statusStyles: Record<string, { label: string; color: string; background: string; border: string }> = {
  PUBLISHED: { label: "Publicado", color: "#10b981", background: "rgba(16,185,129,0.12)", border: "rgba(16,185,129,0.28)" },
  SCHEDULED: { label: "Agendado", color: "#f59e0b", background: "rgba(245,158,11,0.12)", border: "rgba(245,158,11,0.28)" },
  PUBLISHING: { label: "Publicando", color: "#38bdf8", background: "rgba(56,189,248,0.12)", border: "rgba(56,189,248,0.28)" },
  DRAFT: { label: "Rascunho", color: "#a3a3a3", background: "rgba(163,163,163,0.10)", border: "rgba(163,163,163,0.24)" },
  FAILED: { label: "Falhou", color: "#ef4444", background: "rgba(239,68,68,0.12)", border: "rgba(239,68,68,0.28)" },
};
...
export default function DashboardPage() {
  ...
  const [toastStage, setToastStage] = useState<"hidden" | "entering" | "visible" | "exiting">("hidden");
  ...
  useEffect(() => {
    if (!recentPost || !statusMeta) {
      setToastStage("hidden");
      return;
    }

    setToastStage("entering");
    const openTimer = window.setTimeout(() => setToastStage("visible"), 80);
    const hideTimer = window.setTimeout(() => setToastStage("exiting"), 4700);
    const cleanupTimer = window.setTimeout(() => setToastStage("hidden"), 5200);
  ...
  }, [recentPost?.id, recentPost?.status, statusMeta]);
  ...
  return (
    <>
      ...
      {recentPost && statusMeta && toastStage !== "hidden" && (
        <div
          className={`status-popup status-popup--floating status-popup--${toastStage}`}
          style={{ background: statusMeta.background, borderColor: statusMeta.border }}
        >
          <div className="status-popup__head">
            <span className="status-dot" style={{ background: statusMeta.color }} />
            <small>ATUALIZAÇÃO DE STATUS</small>
          </div>
      ...
      <style>{`
        ...
        .console{padding:18px;border:1px solid rgba(255,255,255,.1);border-radius:20px;background:rgba(12,12,13,.72);backdrop-filter:blur(12px);box-shadow:0 18px 45px rgba(0,0,0,.3)}.console-head{display:flex;justify-content:space-between;padding-bottom:14px;color:#777;border-bottom:1px solid #282626}.console-head span{display:flex;align-items:center;gap:7px;font-size:9px;font-weight:800;letter-spacing:.12em}.console-head i,.live i{width:6px;height:6px;border-radius:50%;background:#ff5a0a;box-shadow:0 0 12px #ff5a0a}.console-status{display:flex;align-items:center;gap:12px;padding:18px 0}.console-status>b{width:42px;height:42px;display:grid;place-items:center;color:#ff6a21;border:1px solid rgba(255,90,10,.2);border-radius:13px;background:rgba(255,90,10,.1)}.console-status div{display:grid;gap:3px}.console-status small{color:#777;font-size:9px;letter-spacing:.11em}.console-status strong{font-size:15px}.console-lines{display:grid;gap:1px;overflow:hidden;border:1px solid #292727;border-radius:12px;background:#292727}.console-lines div{padding:10px 12px;display:flex;justify-content:space-between;background:#151516;color:#888;font-size:11px}.console-lines b{color:#fff}.status-popup{padding:14px;border-radius:16px;border:1px solid transparent}.status-popup--floating{position:fixed;right:20px;top:92px;z-index:50;max-width:320px;width:min(320px,calc(100vw - 40px));box-shadow:0 20px 50px rgba(0,0,0,.35);backdrop-filter:blur(14px);transform-origin:top right}.status-popup--entering{animation:toastIn .32s cubic-bezier(.16,1,.3,1) forwards}.status-popup--visible{opacity:1;transform:translateY(0) scale(1)}.status-popup--exiting{animation:toastOut .26s ease-in forwards}.status-popup__head{display:flex;align-items:center;gap:8px}.status-popup__head small{color:#fff;font-size:9px;font-weight:800;letter-spacing:.12em}.status-dot{width:9px;height:9px;border-radius:50%;box-shadow:0 0 0 4px rgba(255,255,255,.04)}.status-popup strong{display:block;margin-top:10px;font-size:15px}.status-popup p{margin-top:6px;color:#d4d4d4;font-size:11px;line-height:1.5}.status-popup span{display:block;margin-top:8px;color:#9ca3af;font-size:10px}
        ...
        html.light .metric-card,html.light .flow-panel{box-shadow:0 12px 38px rgba(33,24,18,.06)}html.light .action-card{background:#fffdfb}@keyframes glow{50%{transform:translate(-18px,18px) scale(1.08);opacity:.75}}@keyframes pulse{50%{transform:scale(.94);opacity:.45}}@keyframes toastIn{0%{opacity:0;transform:translate3d(20px,-12px,0) scale(.96)}60%{opacity:1;transform:translate3d(0,0,0) scale(1.02)}100%{opacity:1;transform:translate3d(0,0,0) scale(1)}}@keyframes toastOut{from{opacity:1;transform:translate3d(0,0,0) scale(1)}to{opacity:0;transform:translate3d(20px,-8px,0) scale(.96)}}@media(max-width:1100px){.command-hero{grid-template-columns:1fr}.console{max-width:560px}.metrics{grid-template-columns:repeat(2,1fr)}.dash-bottom{grid-template-columns:1fr}}@media(max-width:720px){.command-hero{padding:28px 22px;border-radius:22px}.hero-copy h1{font-size:38px}.metrics,.action-grid{grid-template-columns:1fr}.metric-card{min-height:140px}.flow-panel{padding:16px}.action-card{min-height:150px}.status-popup--floating{top:82px;right:12px;width:calc(100vw - 24px)}}@media(prefers-reduced-motion:reduce){.command-hero:before,.hero-rings,.pulse span,.pulse i{animation:none}.status-popup--entering,.status-popup--exiting{animation:none}}
      `}</style>
    </>
  );
}
